...
if [ -f /etc/upwatch/upwatch-auth.log ]
then
	auth=($(cat /etc/upwatch/nq-auth.log))
else
...
if [ -e /etc/upwatch/upwatch-data.log ]
then
	data=($(cat /etc/upwatch/upwatch-data.log))
...
# System load cache
echo "$time $cpu $io $idle $rx $tx" > /etc/upwatch/upwatch-data.log
...
# Get network latency
ping_eu=$(prep $(num "$(ping -c 2 -w 2 ping-eu.upwat.ch | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_us=$(prep $(num "$(ping -c 2 -w 2 ping-us.upwat.ch | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_as=$(prep $(num "$(ping -c 2 -w 2 ping-as.upwat.ch | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
...
# API Request
wget -q -o /dev/null -O /etc/upwatch/upwatch-agent.log -T 60 --post-data "$data_post" --no-check-certificate https://api.upwat.ch/api/agent.json
